...
fn parse_sentence(p: Pair<Rule>) -> Sentence {
    ...
    for field in p.into_inner() {
        match field.as_rule() {
            Rule::viet => sentence.viet = field.as_str().trim().to_string(),
            Rule::eng => sentence.eng = field.as_str().trim().to_string(),
...
fn parse_def(p: Pair<Rule>) -> Sense {
    ...
    for field in p.into_inner() {
        match field.as_rule() {
            Rule::part_of_speech => definition.part_of_speech = field.as_str().trim().to_string(),
            Rule::definition => definition.definition = field.as_str().trim().to_string(),
...
pub fn parse_entry(raw: &str) -> Entry {
    ...
    for field in data.into_inner() {
        match field.as_rule() {
            Rule::word => entry.word = field.as_str().trim().to_string(),
